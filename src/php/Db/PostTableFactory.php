<?php
	...
	class PostTableFactory
	{
		...
		static public function factory( $args )
		{
			...
			if( isset( static::$customPostTypes[ $customPostTypeName ] ) )
			{
				if( is_array( static::$customPostTypes[ $customPostTypeName ] ) )
				{
					static::$customPostTypes[ $customPostTypeName ] = new PostTable( static::$customPostTypes[ $customPostTypeName ], false );
				}

				return static::$customPostTypes[ $customPostTypeName ];
			}
			...
			foreach( $args['defaultFields'] as $defaultFieldID => $defaultFieldName )
			{
				...
				if( !isset( static::$defaultFields[ $defaultFieldID ] ) )
					continue;

				// save default field data
				$defaultFields[ $defaultFieldName ] = static::$defaultFields[ $defaultFieldID ];

				if( in_array( $defaultFieldID, array( 'date', 'date_modified' ) ) )
				{
					$defaultFields[ $defaultFieldName . '_gmt' ] = static::$defaultFields[ $defaultFieldID . '_gmt' ];
				}
			...
			if( !empty( static::$customPostTypes ) )
			{
				$duplicateAlias = false;

				foreach( static::$customPostTypes as $predefinedPostType => $predefinedPostTypeObject )
				{
			...
			$postType = new PostTable( $args );

			// save object in cache
			static::$customPostTypes[ $customPostTypeName ] = $postType;
		...
		{
			return array_keys( static::$customPostTypes );
